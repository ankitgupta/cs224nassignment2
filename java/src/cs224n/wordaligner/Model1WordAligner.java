...
//	private double epsilon = 0.1; //Probability of choosing french sentence of a particular length 
	...
	public Alignment alignSentencePair(SentencePair sentencePair) {
		...
		for (int frenchPosition = 0; frenchPosition < numFrenchWords; frenchPosition++) {
			...
			for (int englishPosition = 0; englishPosition < numEnglishWords; englishPosition++) {
				value = (1.0/numEnglishWords)*ProbabilityMap.getCount(sentencePair.getFrenchWords().get(frenchPosition), sentencePair.getEnglishWords().get(englishPosition));
				if(max < value) {
					max = value;
	...
	public double getAlignmentProb(List<String> targetSentence,
		...
		for (int frenchPosition = 0; frenchPosition < numFrenchWords; frenchPosition++) {
			sum=0;
			for (int englishPosition = 0; englishPosition < numEnglishWords; englishPosition++) {
				sum+=(1.0/numEnglishWords)*ProbabilityMap.getCount(sourceSentence.get(frenchPosition), targetSentence.get(englishPosition));
			}
		...
		for(Pair<Integer, Integer> p : alignment.getSureAlignments()) {
			if(p.getFirst()>=0) 
				probability += Math.log((1.0/numEnglishWords)*ProbabilityMap.getCount(sourceSentence.get(p.getSecond()), targetSentence.get(p.getFirst())));
			else
...
//			}
			...
			for(SentencePair pair : trainingPairs){
				...
				for (int frenchPosition = 0; frenchPosition < numFrenchWords; frenchPosition++) {
					...
					denominator = 0;
					for (int englishPosition = 0; englishPosition < numEnglishWords; englishPosition++) {
						denominator+=ProbabilityMap.getCount(source, pair.getEnglishWords().get(englishPosition));
					}
					denominator+=ProbabilityMap.getCount(source, NULL_WORD);
					for (int englishPosition = 0; englishPosition < numEnglishWords; englishPosition++) {
						CountMap.incrementCount(pair.getEnglishWords().get(englishPosition), source, (ProbabilityMap.getCount(source, pair.getEnglishWords().get(englishPosition))/denominator));
					}
					CountMap.incrementCount(NULL_WORD, source, (ProbabilityMap.getCount(source, NULL_WORD)/denominator));
				}
			}
			// M Step
			for(String target : CountMap.keySet()) {
